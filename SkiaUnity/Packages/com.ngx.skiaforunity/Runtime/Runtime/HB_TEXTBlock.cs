...
namespace SkiaSharp.Unity.HB {
	...
	public class HB_TEXTBlock : MonoBehaviour, ILayoutElement {
		...
		protected SKTypeface skTypefaceBold;
		protected SKTypeface skTypefaceItalic;
		protected SKTypeface skTypefaceBoldItalic;
		protected SKTypeface runtimeTypeface;
		protected SKTypeface runtimeTypefaceBold;
		protected SKTypeface runtimeTypefaceItalic;
		protected SKTypeface runtimeTypefaceBoldItalic;
		private int _runtimeFontVersion;
		...
		private int _typefaceCacheKeyBold;
		private bool _typefaceCacheRegisteredBold;
		private int _typefaceCacheKeyItalic;
		private bool _typefaceCacheRegisteredItalic;
		private int _typefaceCacheKeyBoldItalic;
		private bool _typefaceCacheRegisteredBoldItalic;
		...
		private int _cachedFontMapperVariantKey;
		...
		/// <summary>
		/// Assign runtime font bytes directly (eg. from file picker result).
		/// Pass optional style variants to guarantee bold/italic typefaces.
		/// This clears the inspector "font" reference and uses runtime faces.
		/// </summary>
		public virtual bool SetRuntimeFontBytes(byte[] regular, byte[] bold = null, byte[] italic = null, byte[] boldItalic = null) {
			if (regular == null || regular.Length == 0) return false;

			var main = CreateTypefaceFromBytes(regular);
			if (main == null) return false;

			var b = CreateTypefaceFromBytes(bold);
			var i = CreateTypefaceFromBytes(italic);
			var bi = CreateTypefaceFromBytes(boldItalic);

			DisposeRuntimeTypefaces();
			runtimeTypeface = main;
			runtimeTypefaceBold = b;
			runtimeTypefaceItalic = i;
			runtimeTypefaceBoldItalic = bi;
			_runtimeFontVersion++;

			// Runtime font source takes precedence.
			font = null;
			ReUpdate();
			return true;
		}
		...
		/// <summary>
		/// Assign runtime font files from absolute file paths.
		/// </summary>
		public virtual bool SetRuntimeFontFromPath(string regularPath, string boldPath = null, string italicPath = null, string boldItalicPath = null) {
			if (string.IsNullOrWhiteSpace(regularPath) || !File.Exists(regularPath)) return false;

			byte[] regular = File.ReadAllBytes(regularPath);
			byte[] bold = !string.IsNullOrWhiteSpace(boldPath) && File.Exists(boldPath) ? File.ReadAllBytes(boldPath) : null;
			byte[] italic = !string.IsNullOrWhiteSpace(italicPath) && File.Exists(italicPath) ? File.ReadAllBytes(italicPath) : null;
			byte[] boldItalic = !string.IsNullOrWhiteSpace(boldItalicPath) && File.Exists(boldItalicPath) ? File.ReadAllBytes(boldItalicPath) : null;
			return SetRuntimeFontBytes(regular, bold, italic, boldItalic);
		}

		/// <summary>
		/// Clears runtime font source and falls back to inspector/system font flow.
		/// </summary>
		public virtual void ClearRuntimeFont() {
			DisposeRuntimeTypefaces();
			_runtimeFontVersion++;
			ReUpdate();
		}

		private static SKTypeface CreateTypefaceFromBytes(byte[] bytes) {
			if (bytes == null || bytes.Length == 0) return null;
			SKData data = SKData.CreateCopy(bytes);
			SKTypeface face = SKTypeface.FromData(data);
			data.Dispose();
			return face;
		}

		private void DisposeRuntimeTypefaces() {
			runtimeTypeface?.Dispose();
			runtimeTypeface = null;
			runtimeTypefaceBold?.Dispose();
			runtimeTypefaceBold = null;
			runtimeTypefaceItalic?.Dispose();
			runtimeTypefaceItalic = null;
			runtimeTypefaceBoldItalic?.Dispose();
			runtimeTypefaceBoldItalic = null;
		}

		private static byte[] GetFontBytesFromObject(UnityEngine.Object obj) {
			if (obj is HBFontData hb) return hb.fontBytes;
			if (obj is TextAsset ta) return ta.bytes;
			return null;
		}

		private void AcquireTypefaceFromSource(UnityEngine.Object source, ref SKTypeface typeface, ref int cacheKey, ref bool cacheRegistered) {
			if (source == null) return;
			int key = source.GetInstanceID();
			if (!_typefaceCache.ContainsKey(key)) {
				var bytes = GetFontBytesFromObject(source);
			...
			typeface = _typefaceCache[key];
			if (!cacheRegistered || cacheKey != key) {
				if (cacheRegistered && _typefaceRefCounts.ContainsKey(cacheKey)) {
					_typefaceRefCounts[cacheKey]--;
					if (_typefaceRefCounts[cacheKey] <= 0) {
						if (_typefaceCache.TryGetValue(cacheKey, out var oldFace)) {
							oldFace?.Dispose();
							_typefaceCache.Remove(cacheKey);
						}
						_typefaceRefCounts.Remove(cacheKey);
					}
				}
				_typefaceCacheKey = source == font ? key : _typefaceCacheKey;
				cacheKey = key;
		...
		private void ReleaseTypeface(ref SKTypeface typeface, ref int cacheKey, ref bool cacheRegistered) {
			if (cacheRegistered && _typefaceRefCounts.ContainsKey(cacheKey)) {
				_typefaceRefCounts[cacheKey]--;
				if (_typefaceRefCounts[cacheKey] <= 0) {
		...
		protected virtual void RenderText() {
			...
			if (font != null) {
				...
				AcquireTypefaceFromSource(font, ref skTypeface, ref _typefaceCacheKey, ref _typefaceCacheRegistered);

				int variantKey = 0;
				if (font is HBFontData hbFont) {
					if (hbFont.boldVariant != null) {
						variantKey = variantKey * 31 + hbFont.boldVariant.GetInstanceID();
						AcquireTypefaceFromSource(hbFont.boldVariant, ref skTypefaceBold, ref _typefaceCacheKeyBold, ref _typefaceCacheRegisteredBold);
					} else {
						ReleaseTypeface(ref skTypefaceBold, ref _typefaceCacheKeyBold, ref _typefaceCacheRegisteredBold);
					}

					if (hbFont.italicVariant != null) {
						variantKey = variantKey * 31 + hbFont.italicVariant.GetInstanceID();
						AcquireTypefaceFromSource(hbFont.italicVariant, ref skTypefaceItalic, ref _typefaceCacheKeyItalic, ref _typefaceCacheRegisteredItalic);
					} else {
						ReleaseTypeface(ref skTypefaceItalic, ref _typefaceCacheKeyItalic, ref _typefaceCacheRegisteredItalic);
					}

					if (hbFont.boldItalicVariant != null) {
						variantKey = variantKey * 31 + hbFont.boldItalicVariant.GetInstanceID();
						AcquireTypefaceFromSource(hbFont.boldItalicVariant, ref skTypefaceBoldItalic, ref _typefaceCacheKeyBoldItalic, ref _typefaceCacheRegisteredBoldItalic);
					} else {
						ReleaseTypeface(ref skTypefaceBoldItalic, ref _typefaceCacheKeyBoldItalic, ref _typefaceCacheRegisteredBoldItalic);
					}
				} else {
					ReleaseTypeface(ref skTypefaceBold, ref _typefaceCacheKeyBold, ref _typefaceCacheRegisteredBold);
					ReleaseTypeface(ref skTypefaceItalic, ref _typefaceCacheKeyItalic, ref _typefaceCacheRegisteredItalic);
				...
				if (_cachedFontMapper == null || _cachedFontMapperKey != typefaceKey || _cachedFontMapperVariantKey != variantKey) {
					_cachedFontMapper = new FontMapper(skTypeface, skTypefaceBold, skTypefaceItalic, skTypefaceBoldItalic);
					_cachedFontMapperKey = typefaceKey;
					_cachedFontMapperVariantKey = variantKey;
				}
				rs.FontMapper = _cachedFontMapper;
			} else if (runtimeTypeface != null) {
				int typefaceKey = -1; // runtime source key
				int variantKey = _runtimeFontVersion;
				if (_cachedFontMapper == null || _cachedFontMapperKey != typefaceKey || _cachedFontMapperVariantKey != variantKey) {
					_cachedFontMapper = new FontMapper(runtimeTypeface, runtimeTypefaceBold, runtimeTypefaceItalic, runtimeTypefaceBoldItalic);
					_cachedFontMapperKey = typefaceKey;
					_cachedFontMapperVariantKey = variantKey;
				}
				rs.FontMapper = _cachedFontMapper;
		...
		protected virtual void OnDestroy() {
			...
			ReleaseTypeface(ref skTypeface, ref _typefaceCacheKey, ref _typefaceCacheRegistered);
			ReleaseTypeface(ref skTypefaceBold, ref _typefaceCacheKeyBold, ref _typefaceCacheRegisteredBold);
			ReleaseTypeface(ref skTypefaceItalic, ref _typefaceCacheKeyItalic, ref _typefaceCacheRegisteredItalic);
			ReleaseTypeface(ref skTypefaceBoldItalic, ref _typefaceCacheKeyBoldItalic, ref _typefaceCacheRegisteredBoldItalic);
			DisposeRuntimeTypefaces();
		...
		public virtual void RefreshFontFamily() {
			if (font != null) {
				...
				if (font is HBFontData hbFont) {
					if (hbFont.boldVariant != null) AcquireTypefaceFromSource(hbFont.boldVariant, ref skTypefaceBold, ref _typefaceCacheKeyBold, ref _typefaceCacheRegisteredBold);
					if (hbFont.italicVariant != null) AcquireTypefaceFromSource(hbFont.italicVariant, ref skTypefaceItalic, ref _typefaceCacheKeyItalic, ref _typefaceCacheRegisteredItalic);
					if (hbFont.boldItalicVariant != null) AcquireTypefaceFromSource(hbFont.boldItalicVariant, ref skTypefaceBoldItalic, ref _typefaceCacheKeyBoldItalic, ref _typefaceCacheRegisteredBoldItalic);
				}
				...
				_cachedFontMapper = new FontMapper(runtimeTypeface, runtimeTypefaceBold, runtimeTypefaceItalic, runtimeTypefaceBoldItalic);
				_cachedFontMapperKey = -1;
				_cachedFontMapperVariantKey = _runtimeFontVersion;
				if (rs != null) rs.FontMapper = _cachedFontMapper;
			}
		}
	}
}
